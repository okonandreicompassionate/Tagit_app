-- Tagit — blocking and reporting.
--
-- Adapted from the draft at docs/planned/v3/safety-schema.sql. A social app
-- for teenagers where strangers scan each other in person needs a way to
-- make someone go away, and Snap's developer policies expect exactly that.
--
-- Design points worth keeping straight:
--  * Blocking is enforced in the database, not the client — a blocked pair
--    cannot create a link even if a modified app tries, because the rule
--    lives in a trigger rather than in a screen.
--  * The blocked person is never told. Telling them invites the escalation
--    blocking exists to prevent.
--  * Reports are readable only by their own reporter from the client —
--    the moderation queue is read by whoever moderates, through a server
--    holding the service key, not through the public API.
...
-- the blocked person: telling someone they've been blocked invites exactly
-- the escalation blocking exists to prevent.
...
/* ---------- extend account deletion to cover both new tables ---------- */

-- Supersedes the version in 20260914120000_account_deletion.sql, which
-- predates blocks/reports and left this as an explicit TODO.
